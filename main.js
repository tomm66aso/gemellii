...
/* ═══════════════════════════════════════════════════════════════════════════
   LUXURY RUNTIME — Cursor · Smooth scroll · Breathe observer · Cursor wipe
   ═══════════════════════════════════════════════════════════════════════ */
...
(function(){
  ...
  const cur = document.createElement('div');
  cur.className = 'cur';
  cur.innerHTML = '<div class="cur__dot"></div><div class="cur__ring"></div>';
  document.body.appendChild(cur);
  const dot = cur.querySelector('.cur__dot');
  const ring = cur.querySelector('.cur__ring');
...
/* ── Breathe images observer (relax scale on entry) ── */
(function(){
  // Add breathe class to key image containers
  document.querySelectorAll(
    '.split__img, .food-item, .gateway__bg, .page-hero__bg img'
  ).forEach(el => el.classList.add('breathe'));
  ...
  document.querySelectorAll('.breathe, .reveal, .reveal-img, .clip-reveal, .word-reveal').forEach(el=>ro.observe(el));
})();
...
(function(){
  ...
  if(!vid) return;
  vid.muted = true;
  const play = ()=>{ const p=vid.play(); if(p) p.catch(()=>vid.style.display='none'); };
  vid.readyState >= 3 ? play() : vid.addEventListener('canplay', play, {once:true});
})();
...
(function(){
  ...
  while(document.body.firstChild && document.body.firstChild!==w)
  ...
  let sY=0, cY=0;
  window.addEventListener('wheel', e=>{
    sY = Math.max(0, Math.min(sY+e.deltaY, w.scrollHeight-window.innerHeight));
    requestAnimationFrame(function f(){
      cY += (sY-cY)*.1;
      w.style.transform = `translateY(-${cY}px)`;
      if(Math.abs(sY-cY)>.1) requestAnimationFrame(f);
    });
  },{passive:true});
  window.addEventListener('touchstart',()=>{
    document.documentElement.style.overflow='';
    document.body.style.overflow='';
    w.style.transform=''; w.style.position='';
  },{once:true});
})();
